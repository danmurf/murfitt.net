...
function getPostPathSegments(filePath: string | undefined): string[] {
  return (
    filePath
      ...
      .slice(0, -1)
      // Preserve leading/trailing hyphens: some legacy slugs (e.g.
      // "essential-firefox-web-developer-") rely on them for URL parity.
      .map(segment => slugify(segment, { lower: true, trim: false })) ?? []
  );
}
